...
if(novoX >= 0 && novoX < CONFIGURACOES.largura && novoY >= 0 && novoY < CONFIGURACOES.altura && inimigos.estaNaPosicao(novoX, novoY) === false){
inimigo.x = novoX
inimigo.y = novoY
}
if(novoY >= 0 && novoY < CONFIGURACOES.altura){
...
mover: mover,
estaNaPosicao: (x, y) => {
for(var i =0; i< inimigo.lista.length; i++){
        if(inimigo.lista[i].x === x && inimigo.lista[i].y === y){
            return true;
        }
    } 
}
